...
//
// VERSION = 0.10, 15 January 2019
//
// Revision history:
// 
// 0.10 - Changed parameters passed to "renderer" functions to match those
//        passed by SlickGrid to "formatter" functions.
// 0.09 - Mary McShane added colstyle parameter.
...
;(function($, window, document, undefined) {
	...
	$.fn.accessibleGrid = function(colDesc, options, aData) {
        ...
        // 1. row - ZERO-based index of the row in "aData" containing "value".
        // 2. cell - ZERO-based index of the column in "aData" containing "value".
        // 3. value  - the value of the data cell in question
        // 4. columnDef - 
        // 5. dataContext - the entire record in which "value" resides, 
        //                  i.e., the row in "aData" containing "value".
		...
		$.each(aData, function(ndx, record) {
			...
			for (i = 0; i < colDesc.length; i++) {
				if (i === 0) {
                    ...
                    if (colDesc[i].colstyle !== undefined) {              // Added to support right-justifying data in columns, not just header
						szTd += ' style="' + colDesc[i].colstyle + '"';                         
					}
				...
				szRow += (colDesc[i].renderer === undefined) ? record[colDesc[i].dataIndex] 
															 : colDesc[i].renderer(count-1,                         // Convert to zero-based row index
                                                                                   i,                               // Zero-based column index
                                                                                   record[colDesc[i].dataIndex],    // value
                                                                                   colDesc[i],                      // columnDef
                                                                                   record);                         // Entire row of data
